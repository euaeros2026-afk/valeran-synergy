...
// Global ÃÂ¢ÃÂÃÂ readable by all callbacks without closure issues
...
export default function Chat({ supabase, partner }) {
  ...
  // Keep global in sync ÃÂ¢ÃÂÃÂ no closure capture needed
  ...
  useEffect(() => {
    ...
    // REALTIME: read window.__valeranUser ÃÂ¢ÃÂÃÂ never stale, no closure
    const ch = supabase.channel('sv_chat_v6')
      .on('postgres_changes', { event:'INSERT', schema:'public', table:'chat_messages' }, function(payload) {
        ...
        setMessages(function(prev) {
          ...
          // Someone else's message ÃÂ¢ÃÂÃÂ just add it
  ...
  async function sendMessage() {
    ...
    var full=text
    if(replyTo){full='[Re: '+replyTo.senderName+': '+replyTo.content.slice(0,60)+(replyTo.content.length>60?'...':'')+' > '+text;setReplyTo(null)}
    var isAI = /^(valeran|valera)[,\s!?.]/i.test(text) || /^\u0432\u0430\u043b\u0435\u0440\u0430[,\s]/i.test(text)
    addTemp(full)
    try{
      var t=await getToken()
      if(isAI){
  ...
  async function sendPhoto(file){
    ...
    addTemp('ÃÂ°ÃÂÃÂÃÂ· '+file.name)
  ...
  async function sendFile(file){
    ...
    addTemp('ÃÂ°ÃÂÃÂÃÂ '+file.name+' ÃÂ¢ÃÂÃÂ analysing...')
  ...
  async function startRec(){
    try{
      ...
      rec.onstop=async function(){
        ...
        setSending(true);addTemp('ÃÂ°ÃÂÃÂÃÂ¤ ...')
        try{var r=await fetch(API+'/api/chat/voice',{method:'POST',headers:{Authorization:'Bearer '+t},body:fd});var d=await r.json();if(d.transcript){setMessages(function(p){var f=[].concat(p);for(var i=f.length-1;i>=0;i--){if(f[i]._tmp&&f[i].content==='ÃÂ°ÃÂÃÂÃÂ¤ ...'){f[i]=Object.assign({},f[i],{content:'ÃÂ°ÃÂÃÂÃÂ¤ "'+d.transcript+'"'});break}}return f})}if(d.reply)addAI(d.reply)}catch(e){setError('Voice failed')}finally{setSending(false);stream.getTracks().forEach(function(t){t.stop()})}
      }
  ...
  var EMOJIS=['ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ¢ÃÂÃÂ¤ÃÂ¯ÃÂ¸ÃÂ','ÃÂ°ÃÂÃÂÃÂ¥','ÃÂ¢ÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂª','ÃÂ°ÃÂÃÂÃÂ¯','ÃÂ°ÃÂÃÂÃÂ¦','ÃÂ°ÃÂÃÂÃÂ°','ÃÂ°ÃÂÃÂÃÂ­','ÃÂ°ÃÂÃÂ¤ÃÂ','ÃÂ¢ÃÂÃÂ¡','ÃÂ°ÃÂÃÂÃÂ¨ÃÂ°ÃÂÃÂÃÂ³','ÃÂ°ÃÂÃÂÃÂªÃÂ°ÃÂÃÂÃÂº','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ¡','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂ¤ÃÂ','ÃÂ°ÃÂÃÂÃÂ¯','ÃÂ¢ÃÂ­ÃÂ','ÃÂ°ÃÂÃÂÃÂ¸','ÃÂ°ÃÂÃÂÃÂ','ÃÂ°ÃÂÃÂÃÂ®','ÃÂ°ÃÂÃÂÃÂ']
    ...
    <div className="chat-page">
      ...
      {tab==='dash' && (
        <div className="dashboard">
          ...
          <div className="dash-card">
            ...
            {[{ph:'Phase 1',d:'Apr 15-19',c:'Electronics ÃÂÃÂ· Hardware ÃÂÃÂ· Lighting ÃÂÃÂ· Tools',col:'#e8a045'},{ph:'Phase 2',d:'Apr 23-27',c:'Home Goods ÃÂÃÂ· Ceramics ÃÂÃÂ· Furniture ÃÂÃÂ· Gifts',col:'#7c6af7'},{ph:'Phase 3',d:'May 1-5',c:'Fashion ÃÂÃÂ· Textiles ÃÂÃÂ· Toys ÃÂÃÂ· Personal Care',col:'#4ade80'}].map(function(p){return(
              <div key={p.ph} style={{display:'flex',gap:10,padding:'8px 0',borderBottom:'1px solid rgba(255,255,255,0.06)'}}>
                ...
                <div><div style={{fontWeight:700,fontSize:13,color:p.col}}>{p.ph} <span style={{fontWeight:400,color:'rgba(255,255,255,0.45)'}}>ÃÂÃÂ· {p.d}</span></div><div style={{fontSize:11,color:'rgba(255,255,255,0.4)',marginTop:2}}>{p.c}</div></div>
              </div>
            )})}
          </div>
          <div className="dash-card">
            <div className="dash-card-title">Team</div>
            {[{n:'Alexander Oslan',r:'Owner ÃÂÃÂ· EN'},{n:'Ina Kanaplianikava',r:'Partner ÃÂÃÂ· RU'},{n:'Konstantin Khoch',r:'Partner ÃÂÃÂ· RU'},{n:'Konstantin Ganev',r:'Partner ÃÂÃÂ· BG'},{n:'Slavi Mikinski',r:'Observer ÃÂÃÂ· BG'}].map(function(m){
          ...
          <div className="dash-card">
            ...
            <div className="dash-info-row"><span>ÃÂ°ÃÂÃÂÃÂ¦</span><span>April: 22-28ÃÂÃÂ°C, humid, rain ÃÂ¢ÃÂÃÂ bring umbrella</span></div>
            <div className="dash-info-row"><span>ÃÂ°ÃÂÃÂÃÂ</span><span>CFTC: 4000-888-999 ÃÂÃÂ· +86-20-28-888-999</span></div>
            <div className="dash-info-row"><span>ÃÂ°ÃÂÃÂÃÂ</span><span>cantonfair.org.cn ÃÂÃÂ· Canton Fair APP</span></div>
          </div>
          <div className="dash-card">
            <div className="dash-card-title">Margin Target &gt;35%</div>
            <div style={{fontSize:12,color:'rgba(255,255,255,0.55)',lineHeight:1.8}}>
              <div>Landed = buy ÃÂÃÂ 1.12 (freight) ÃÂÃÂ 1.035 (duty)</div>
              <div>Net = (sell ÃÂ¢ÃÂÃÂ landed ÃÂ¢ÃÂÃÂ 15% fees ÃÂ¢ÃÂÃÂ 10% ads) ÃÂÃÂ· sell</div>
              <div style={{marginTop:6,padding:'6px 10px',background:'rgba(74,222,128,0.08)',borderRadius:8,border:'1px solid rgba(74,222,128,0.2)',color:'#4ade80',fontWeight:600}}>Example: buy $4 ÃÂ¢ÃÂÃÂ sell ÃÂ¢ÃÂÃÂ¬18 ÃÂ¢ÃÂÃÂ margin 51% ÃÂ¢ÃÂÃÂ</div>
            </div>
          </div>
        </div>
      )}
      ...
      {tab==='chat' && (
        <>
          ...
          {replyTo&&(<div style={{background:'rgba(255,255,255,0.06)',borderLeft:'3px solid #e8a045',padding:'6px 12px',display:'flex',alignItems:'center',justifyContent:'space-between',fontSize:12,color:'rgba(255,255,255,0.7)',flexShrink:0}}><div><span style={{color:'#e8a045',fontWeight:600}}>{replyTo.senderName}</span> ÃÂÃÂ· {replyTo.content.slice(0,55)}{replyTo.content.length>55?'...':''}</div><button onClick={function(){setReplyTo(null)}} style={{background:'none',border:'none',color:'rgba(255,255,255,0.5)',cursor:'pointer',fontSize:20,lineHeight:1,padding:'0 4px'}}>ÃÂÃÂ</button></div>)}
          ...
          <div className="chat-input-bar">
            ...
            <input ref={inputRef} className="chat-input" value={input} onChange={handleInput} onKeyDown={function(e){if(e.key==='Enter'&&!e.shiftKey)sendMessage()}} placeholder='"Valeran, ÃÂ¢ÃÂÃÂ¦" for AI ÃÂÃÂ· or just chat' disabled={recording}/>
            <button className="input-action-btn" onClick={function(){setShowEmoji(function(p){return!p})}} style={{fontSize:18}} title="Emoji">ÃÂ°ÃÂÃÂÃÂ</button>
