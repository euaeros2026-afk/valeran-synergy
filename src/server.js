...
app.post('/api/telegram/webhook', async function(req, res) {
  ...
  // Inline AI: get system + save + history in parallel
  ...
  var tgReply = null;
  try { tgReply = await core.callAI(tgMsgs, tgSys, 400, 2500, true); } catch(e) {}
  // Send TG reply FIRST (before res.sendStatus — so Vercel does not kill it)
  if (tgReply) {
    var tgClean = tgReply.replace(/^\*\*[A-Z]{2,3}\*\*[^\n]*\n*/gm,'').replace(/^[A-Z]{2,3}:[^\n]*\n*/gm,'').trim();
    try { var _r=await fetch('https://api.telegram.org/bot'+process.env.TELEGRAM_BOT_TOKEN+'/sendMessage',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chat_id:String(chatId),text:tgClean,reply_to_message_id:msg.message_id})}); var _j=await _r.json(); if(!_j.ok){console.error('[tg]',JSON.stringify(_j));} } catch(e){console.error('[tg err]',e.message);}
    core.saveMessage(sid,'assistant',tgClean,null,'telegram','Valeran').catch(function(){});
  }
  // Respond to Telegram LAST (total time ~2.8s < 5s limit)
  res.sendStatus(200);


app.get('/api/tg-diag', async function(req, res) {
  var token = process.env.TELEGRAM_BOT_TOKEN || 'NOT_SET';
  var r = await fetch('https://api.telegram.org/bot' + token + '/sendMessage', {
    method: 'POST', headers: {'Content-Type':'application/json'},
    body: JSON.stringify({chat_id: '-1003843622106', text: 'DIAG ' + new Date().toISOString().slice(11,19)})
  });
  var j = await r.json();
  res.json({tokenLen: token.length, tokenEnd: token.slice(-6), apiOk: j.ok, msgId: j.result && j.result.message_id, err: j.description || null});
});
